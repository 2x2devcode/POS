#!/usr/bin/env bash
# compile-windows.sh — Cross-compile POS Windows CLI + GUI from Ubuntu 22.04
# Based on https://github.com/2x2devcode/RNRC/blob/master/compile-windows.sh
# Installs host + Qt (mingw) dependencies, then builds BOTH:
#   release/windows/posd.exe
#   release/windows/pos-qt.exe
set -euo pipefail
...
write_error_extract() {
    ...
    {
        echo "=== POS Windows build — error extract ==="
...
# Run apt-get as root or via sudo. Keep DEBIAN_FRONTEND as a shell assignment
# (not as argv) so it works both under root and with sudo.
apt_run() {
    if [[ ${EUID} -eq 0 ]]; then
...
apt_install() {
    ...
    if ((${#missing[@]})); then
        ...
        # Accept Label/Codename changes from third-party PPAs (e.g. ondrej/php)
...
# Run a command as root (noop wrapper when already root).
as_root() {
    if [[ ${EUID} -eq 0 ]]; then
        "$@"
    else
...
setup_mingw_posix() {
    if need_cmd update-alternatives; then
        if [[ -x /usr/bin/${TARGET}-g++-posix ]]; then
            as_root update-alternatives --set "${TARGET}-g++" "/usr/bin/${TARGET}-g++-posix" 2>/dev/null || true
        fi
        if [[ -x /usr/bin/${TARGET}-gcc-posix ]]; then
            as_root update-alternatives --set "${TARGET}-gcc" "/usr/bin/${TARGET}-gcc-posix" 2>/dev/null || true
        fi
    fi
...
install_mxe_qt() {
    ...
    apt_run install -y "$tools_pkg" 2>/dev/null || true
    ...
    ensure_include_limits() {
        ...
        if [[ -n "$after" ]] && grep -qF "$after" "$file"; then
            ...
            awk -v after="$after" '
                ...
                { print }
            ' "$file" > "${file}.pos.tmp" && mv "${file}.pos.tmp" "$file"
...
build_qt_from_source() {
    ...
    # Wallet crypto still links OpenSSL separately via pos-qt.pro (-lssl -lcrypto).
...
build_cli() {
    ...
    verify_file "$ROOT/src/posd.exe" "CLI build failed"
    mkdir -p "$RELEASE_DIR"
    cp -f "$ROOT/src/posd.exe" "$RELEASE_DIR/"
    "${TARGET}-strip" "$RELEASE_DIR/posd.exe" 2>/dev/null || true
    file "$RELEASE_DIR/posd.exe" || true
    verify_no_mingw_runtime_dlls "$RELEASE_DIR/posd.exe"
}
...
build_gui() {
    ...
    local qmake_args=(
        ...
        "QMAKE_LFLAGS+=-static -static-libgcc -static-libstdc++ -Wl,--stack,16777216"
        "LIBS+=${static_rt_libs}"
    )
    ...
    "$qmake_bin" "${qmake_args[@]}" "$ROOT/pos-qt.pro"
    ...
    local exe
    exe="$(find "$bdir" -name 'pos-qt.exe' | head -1 || true)"
    [[ -n "$exe" ]] || exe="$(find "$ROOT" -maxdepth 3 -name 'pos-qt.exe' | head -1 || true)"
    [[ -n "$exe" ]] || die "pos-qt.exe not produced"
    mkdir -p "$RELEASE_DIR"
    cp -f "$exe" "$RELEASE_DIR/"
    if need_cmd "${TARGET}-strip"; then
        "${TARGET}-strip" "$RELEASE_DIR/pos-qt.exe" 2>/dev/null || true
    elif [[ -x "${MXE_PREFIX}/usr/bin/${MXE_TARGET}-strip" ]]; then
        "${MXE_PREFIX}/usr/bin/${MXE_TARGET}-strip" "$RELEASE_DIR/pos-qt.exe" 2>/dev/null || true
    fi
    file "$RELEASE_DIR/pos-qt.exe" || true
    verify_no_mingw_runtime_dlls "$RELEASE_DIR/pos-qt.exe"
}
...
main() {
    ...
    ls -la "$RELEASE_DIR"
    verify_file "$RELEASE_DIR/posd.exe" "CLI artifact missing"
    if [[ "$BUILD_GUI" == "1" ]]; then
        verify_file "$RELEASE_DIR/pos-qt.exe" "GUI artifact missing"
    fi
