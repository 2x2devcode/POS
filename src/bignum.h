...
/** C++ wrapper for BIGNUM (OpenSSL 1.0 / 1.1 / 3.x compatible) */
class CBigNum
{
private:
    BIGNUM* bn;
...
public:
    ...
    */
    ...
    std::string ToString(int nBase=10) const
    {
        ...
        CBigNum dv;
        CBigNum rem;
        if (BN_cmp(bn1.get(), bn0.get()) == 0)
            return "0";
        while (BN_cmp(bn1.get(), bn0.get()) > 0)
        {
            if (!BN_div(dv.get(), rem.get(), bn1.get(), bnBase.get(), pctx))
    ...
    */
    ...
    CBigNum pow(const CBigNum& e) const {
        CAutoBN_CTX pctx;
        CBigNum ret;
        if (!BN_exp(ret.get(), bn, e.get(), pctx))
    ...
    CBigNum mul_mod(const CBigNum& b, const CBigNum& m) const {
        CAutoBN_CTX pctx;
        CBigNum ret;
        if (!BN_mod_mul(ret.get(), bn, b.get(), m.get(), pctx))
    ...
    CBigNum pow_mod(const CBigNum& e, const CBigNum& m) const {
        ...
        if (e < CBigNum(0)) {
            ...
            if (!BN_mod_exp(ret.get(), bn, e.get(), m.get(), pctx))
    ...
    */
    CBigNum inverse(const CBigNum& m) const {
        CAutoBN_CTX pctx;
        CBigNum ret;
        if (!BN_mod_inverse(ret.get(), bn, m.get(), pctx))
    ...
    CBigNum gcd( const CBigNum& b) const{
        CAutoBN_CTX pctx;
        CBigNum ret;
        if (!BN_gcd(ret.get(), bn, b.get(), pctx))
    ...
    */
    ...
    CBigNum& operator--()
    {
        // prefix operator
        CBigNum r;
        if (!BN_sub(r.get(), bn, BN_value_one()))
